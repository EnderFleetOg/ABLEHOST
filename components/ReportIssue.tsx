...
<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 25px; border: 4px solid #A855F7; border-radius: 16px; background-color: #0c0a09; color: #ffffff;">
  <h2 style="color: #2DD4BF; border-bottom: 2px solid #A855F7; padding-bottom: 12px; margin-top: 0; font-size: 24px; text-transform: uppercase; font-style: italic;">ABLE Barrier Report</h2>
  
  <p style="font-size: 16px; color: #dddddd; line-height: 1.5;">A user has submitted a barrier issue report from the ABLE platform.</p>
  ...
  <h3 style="color: #2DD4BF; margin-bottom: 10px; font-size: 18px; text-transform: uppercase;">Barrier Details:</h3>
  <div style="background-color: #1c1917; border-left: 6px solid #A855F7; border-radius: 4px; padding: 20px; font-size: 16px; line-height: 1.6; white-space: pre-wrap; color: #ffffff; font-weight: 500;">
    ${desc}
  </div>
  ...
  <div style="margin-top: 35px; padding-top: 15px; border-top: 1px solid #44403c; text-align: center; font-size: 11px; color: #78716c; text-transform: uppercase; tracking-widest: 0.1em;">
    Autonomous Dispatch • ABLE Ecosystem Platform
      ...
      await sendGmailEmail({
        ...
        subject: `[ABLE Platform] New Barrier Report by ${user?.name || 'Anonymous'}`,
        bodyHtml
      });

      setDone(true);
      speak("Your barrier report has been successfully mailed to enderfleet.ai@gmail.com");
      setTimeout(onClose, 3000);
    } catch (err: any) {
      console.error(err);
      setAuthError(`Delivery failed: ${err.message || 'Unknown integration error occurred'}`);
        ...
        <AnimatePresence mode="wait">
          {done ? (
            ...
            >
              ...
              <h2 className="text-5xl font-black text-ableTeal tracking-tighter uppercase italic">Report Mailed</h2>
              <p className="text-2xl font-bold text-white/60">Adapting the system now.</p>
            ...
            >
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  <p className="text-xl font-bold text-white/60 font-sans">What hindered your ability today?</p>
                  <button 
                ...
                <textarea 
                  ...
                  className="w-full h-48 bg-white/5 border-4 border-white/20 rounded-3xl p-8 text-2xl font-bold text-white outline-none focus:border-ableTeal transition-all resize-none shadow-inner animate-in duration-350"
                />
              </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold text-white/40">
                      {googleUser ? `Connected: ${googleUser.email}` : 'OAuth authorization required to route reports'}
                    </p>
                  </div>
                  {!googleUser ? (
                    <button
                    ...
                    <div className="flex items-center gap-3 bg-white/5 border-2 border-ablePurple/45 px-4 py-2 rounded-2xl">
                      <span className="w-2.5 h-2.5 rounded-full bg-ablePurple animate-pulse"></span>
                      <span className="text-xs font-black text-ablePurple uppercase tracking-widest font-mono">READY</span>
                      {googleUser.photoURL && (
                        <img src={googleUser.photoURL} alt="avatar" className="w-8 h-8 rounded-full border border-ablePurple" referrerPolicy="no-referrer" />
                      )}
                    </div>
                  )}
                </div>
              ...
              <button 
                onClick={submit}
                disabled={loading || !desc.trim() || !googleUser}
